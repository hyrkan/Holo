        ...
        <div class="col-lg-12">
            <div class="card stretch stretch-full">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="card-title">{{ $certificate->id ? 'Edit' : 'Create' }} Default Certificate Template</h5>
                    <div class="card-header-action">
                        <a href="{{ route('admin.settings.certificates.index') }}" class="btn btn-sm btn-light">
                            <i class="feather-arrow-left me-1"></i> Back to Templates
                        </a>
                    </div>
                </div>
                <div class="card-body">
                    <form action="{{ $certificate->id ? route('admin.settings.certificates.update', $certificate) : route('admin.settings.certificates.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $certificate->name) }}" required placeholder="e.g. Standard Attendance Certificate">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $certificate->title ?? 'CERTIFICATE OF ATTENDANCE') }}" required>
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea class="form-control @error('body') is-invalid @enderror" id="body" name="body" rows="4" required>{{ old('body', $certificate->body ?? 'for leur active participation during the event held on [EVENT_DATE] at [EVENT_LOCATION].') }}</textarea>
                                    <div class="form-text">Use <code>[STUDENT_NAME]</code>, <code>[EVENT_NAME]</code>, <code>[EVENT_DATE]</code>, and <code>[EVENT_LOCATION]</code> as placeholders.</div>
                                    @error('body')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            @if($certificate->id)
                                <a href="{{ route('admin.settings.certificates.preview', $certificate) }}" target="_blank" class="btn btn-info">
                                    <i class="feather-eye me-1"></i> Preview Template
                                </a>
                            @endif
                            <button type="submit" class="btn btn-primary">
                                <i class="feather-save me-1"></i> Save Certificate Template
                            </button>
                        </div>
                    </form>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateNumbers() {
        container.querySelectorAll('.signatory-item').forEach((item, i) => {
            const numSpan = item.querySelector('.signatory-num');
